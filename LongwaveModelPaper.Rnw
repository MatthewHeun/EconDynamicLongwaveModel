...
\noindent with $Y_\mathrm{t}$ representing economic output (in 2005\$/year), $A_\mathrm{t}$ representing the stock of knowledge (in units of ideas), $L_\mathrm{Y,t}$ representing labor in the production sector (in workers), and $T_\mathrm{t}$ representing the stock of available land (in m$^2$).
...
\noindent $N_\mathrm{t}$ is the total population (in persons), and $L_\mathrm{t}$ is the total amount of available labor in the economy (in workers). So, $(1-\tau_\mathrm{t})$ represents the fraction of available time spent on childrearing (unitless or time childrearing per total time). It is worth noting that the units of $\tau_\mathrm{t}$ can also be workers/capita, as shown in Equation~\ref{eq:pop_work}.
...
b <- calculatedHistoricalData["B"] / calculatedHistoricalData["N"]; names(b) <- "b" # Births/year-capita
d <- calculatedHistoricalData["D"] / calculatedHistoricalData["N"]; names(d) <- "d" # Deaths/year-capita
...
tau <- calculatedHistoricalData["L"] / calculatedHistoricalData["N"]; names(tau) <- "tau" # Fraction of workers in each year
w_worker <- calculatedHistoricalData["w_capita"] / tau; names(w_worker) <- "w_worker" # 2005$/year-worker
c <- w_worker * tau; names(c) <- "c" # consumption in 2005$/year-capita. Same as wages in 2005$/year-capita.
calculatedHistoricalData <- cbind(calculatedHistoricalData, tau, w_worker, c)
@

Using Equation \ref{eq:pop_work}, we calculated historical values for $\tau_\mathrm{t}$. Per capita income data were divided by $\tau_\mathrm{t}$ to obtain a wage time series ($w_\mathrm{t}$) in units of 2005\$/year-worker. Using the historical $\tau_\mathrm{t}$ values and Equation \ref{eq:consumption_constraint}, we calculated historical values of consumption ($c_\mathrm{t}$) in units of 2005\$/year-capita. Note that consumption ($c_\mathrm{t}$) is equal to per-capita wages.

<<calculate_nu_pi_and_zeta>>=
nu <- calculatedHistoricalData["b"] / (1.0 - calculatedHistoricalData["tau"]); names(nu) <- "nu" # Historical nu
nu_0 <- nu[firstYear, "nu"]
pi <- calculatedHistoricalData["L_A"] / calculatedHistoricalData["L"]; names(pi) <- "pi" # Historical pi
pi_0 <- pi[firstYear, "pi"]

# Problems here. 
# 
# 1) the line below (using "w_worker") works fine, but gives the answer that zeta_1980 = 1.52, whereas Caleb 
#    obtained zeta_1980 = 0.67. I think the difference is caused by the fact that Caleb erroneously used "w_capita" 
#    instead of "w_worker" in the EES sheet named "JonesModelGroundTruthParameters.EES".
# 2) when I use "w_capita" (to verify that I obtain Caleb's values with the erroneous variable), I get NA values 
#    for the data.frame wALA. This is weird and surprising. Both "w_capita" and "w_worker" are column titles in 
#    the calculatedHistoricalData data.frame.  As such, they should be available for calculations. 
#    I don't know why this is happening.

wALA <- calculatedHistoricalData["w_worker"] * calculatedHistoricalData["L_A"]; names(wALA) <- "wALA"
# wALA <- calculatedHistoricalData["w_capita"] * calculatedHistoricalData["L_A"]; names(wALA) <- "wALA"
...
We used $\tau_\mathrm{1980}$ and $b_\mathrm{1980}$ with Equation~\ref{eq:birth_constraint} to calculate a value for $\nu_{1980}$ (\Sexpr{nu_0}~births/year-capita) that is held constant through time. We used $L_\mathrm{A,1980}$ and $L_{1980}$ with Equation~\ref{eq:pi} to calculate a unitless value for $\pi$ (\Sexpr{pi_0}) that is held constant through time. $\pi$ was used along with the historical wage and labor data to calculate an invariant value for $\zeta$ (0.6794) **** Heun Write R code to calculate these numbers. ****
% **** SEE JonesModelGroundTruthParameters.EES IN THE REPOSITORY ****

Following \citet[p.~16]{Jones:2001wn}, we take $\bar{b} = 0$, because it is the asymptotic birth rate as consumption goes to infinity, and people will be inclined towards not having children at infinite consumption. ****Heun see pg. 16 of Jones at the bottom if you want to check this out for yourself****
...
dataXtable <- xtable(x=calculatedHistoricalData[, 10:21], caption="Historical data for the U.S.", label=paste("tab:Historical_US_Data"))
...
rawDataXtable <- xtable(x=usDataMassaged, 
                        caption="U.S. raw historical data. GDP ($Y_\\mathrm{t}$) in billion 2005\\$/year; wages ($w_\\mathrm{t}$) in 2005\\$/year-capita; total population ($L_\\mathrm{t}$) in millions of workers; researchers in STEM disciplines ($L_\\mathrm{A,t}$) in millions of workers; births ($B_\\mathrm{t}$) and deaths ($D_\\mathrm{t}$) in millions/year; population ($N_\\mathrm{t}$) in millions (capita); capital stock ($K_\\mathrm{t}$) in billion 2005\\$; and energy ($E_\\mathrm{t}$) in TJ/year.",
