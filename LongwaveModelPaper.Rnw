...
names(z) <- "z" 
calculatedHistoricalData <- cbind(calculatedHistoricalData, z)
@
...
dataXtable <- xtable(x=calculatedHistoricalData[, 10:21], 
...
********* Heun ended here. Need to perform this parameter estimation **********
...
To determine $\eta$, $\gamma$, and $G$ we also used the 
