...
d_0 <- calculatedHistoricalData[firstYear, "d"]
...
Equation~\ref{eq:mortality_rate} gives
...
$\omega_1~=~\Sexpr{pprint(omega_1, digits=1)}$, 
$\omega_2~=~\Sexpr{pprint(omega_2, digits=3)}$, and 
$\omega_3~=~\Sexpr{pprint(omega_3, digits=1)}$.
$\omega_1$, $\omega_2$, and $\omega_3$ are all 
significant at the 1\% level. Figure~**** compares predicted
and actual mortality rates.

<<mortality_Graph, fig.pos="H", fig.width=maxWidth, fig.height=4, fig.cap="Mortality model results.">>=
