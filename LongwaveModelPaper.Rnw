...
ensures that year $\mathrm{t} = \mathrm{t}_0$ will be matched. 
Solving for $\tilde{b}$ gives
...
\noindent which can be used in the \texttt{R} \citep{R} function 
\texttt{nls} to estimate 
$\eta=\Sexpr{pprint(eta, digits=3)}$, and
$\gamma=\Sexpr{pprint(gamma, digits=3)}$. Both parameters 
