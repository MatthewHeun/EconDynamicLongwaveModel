...
w_worker <- calculatedHistoricalData["w_capita"] / tau; names(w_worker) <- "w_worker" # wage/year-worker
c <- w_worker * tau; names(c) <- "c" # consumption per year-capita. Same as wages/year-capita.
...
calculatedHistoricalData <- cbind(calculatedHistoricalData, nu, pi)
@

We used $\tau_\mathrm{1980}$ and $b_\mathrm{1980}$ with Equation~\ref{eq:birth_constraint} to calculate a value for $\nu_{1980}$ (\Sexpr{nu_0}~births/year-capita) that is held constant through time. We used $L_\mathrm{A,1980}$ and $L_{1980}$ with Equation~\ref{eq:pi} to calculate a value for $\pi$ (\Sexpr{pi_0}) that is held constant through time. $\pi$ was used along with the historical wage and labor data to calculate an invariant value for $\zeta$ (0.6794) **** Heun Write R code to calculate these numbers. ****
...
rawDataXtable <- xtable(x=usDataMassaged, 
                        caption="U.S. raw historical data. GDP ($Y_\\mathrm{t}$) in billion 2005\\$/year; wages ($w_\\mathrm{t}$) in 2005\\$/year-capita; total population ($L_\\mathrm{t}$), researchers in STEM disciplines ($L_\\mathrm{A,t}$), births ($B_\\mathrm{t}$), and deaths ($D_\\mathrm{t}$) in millions/year; population ($N_\\mathrm{t}$) in millions; capital stock ($K_\\mathrm{t}$) in billion 2005\\$; and energy ($E_\\mathrm{t}$) in TJ/year.",
