...
nu_1980 <- nu[firstYear, "nu"]
pi <- calculatedHistoricalData["L_A"] / calculatedHistoricalData["L"]; names(pi) <- "pi" # Historical pi
pi_1980 <- pi[firstYear, "pi"]

# Problems here. 
# 
# 1) the line below (using "w_worker") works fine, but gives the answer that 
#    zeta_1980 = 1.52, whereas Caleb obtained zeta_1980 = 0.67. 
#    I think the difference is caused by the fact that Caleb erroneously 
#    used "w_capita" instead of "w_worker" in the EES sheet named 
#   "JonesModelGroundTruthParameters.EES".
# 2) when I use "w_capita" (to verify that I obtain Caleb's values with the erroneous variable), 
#    I obtain 0.6794, same as Caleb.
...
zeta_1980 <- zeta[firstYear, "zeta"]
calculatedHistoricalData <- cbind(calculatedHistoricalData, nu, pi, zeta)
@

We used $\tau_\mathrm{1980}$ and $b_\mathrm{1980}$ with Equation~\ref{eq:birth_constraint} to calculate a value for $\nu_{1980}$ (\Sexpr{nu_1980}~births/year-capita) that is held constant through time. We used $L_\mathrm{A,1980}$ and $L_{1980}$ with Equation~\ref{eq:pi} to calculate a unitless value for $\pi$ (\Sexpr{pi_1980}) that is held constant through time. Equation~\ref{eq:knowledge_compensation} was used with $\pi_\mathrm{1980}$ and wage and labor data for 1980 in to calculate an invariant value for $\zeta$ (\Sexpr{zeta_1980}) **** Heun Write R code to calculate these numbers. ****
