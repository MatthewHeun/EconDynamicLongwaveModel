...
firstYear <- "1980"
Y_0  <- data[firstYear, "Y"]
N_0 <- data[firstYear, "N"]
L_Y_0 <- L_Y[firstYear,"L_Y"]
y <- data["Y"] / Y_0; names(y) <- "y"
n <- data["N"] / N_0; names(n) <- "n"
l_Y <- (data["L"] - data["L_A"]) / L_Y_0; names(l_Y) <- "l_Y"

data <- cbind(data, L_Y, y, n, l_Y)
...
dataXtable <- xtable(x=data[, 10:13], caption="Data for the U.S.", label=paste("tab:US_Data"))
