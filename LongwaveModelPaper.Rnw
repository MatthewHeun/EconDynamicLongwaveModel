...
\usepackage{float} % Provides precise positioning of figures
...
singleGraphWidth <- maxWidth
singleGraphHeight <- 4.5 #Inches
...
Make sure here that readers understand why we need the Jones model 
to be reproduced and why for such a short time frame. 
I do not think you are that interested in the demographic transition, 
but you are interested in the interplay between the stock of ideas and 
certain limiting factors such as  labour, capital, energy, land. 
New work on energy growth modelling may provide some insights 
into what the real limiting factors for the US were in the period 1980-2011, 
and speculate on what the limiting factors may become in the future. 
When adding energy to the US economic production function 
over the 1980-2011 time frame we must be able to show that 
it is indeed valuable to do so--getting better predictive power, for example. 
It is a pity the energy shock data from the 1970s is missed 
with a 1980-2011 time frame. Can that still be included? 
We need to find an "energy signal," even if 
it is embedded in other factors of production--a kind of nested function, 
if energy limits and innovation potential to circumvent those remains our thesis.
****
...
$k_\mathrm{c}$, $\mu$, and $\nu$ are constant with respect 
to time, we grouped these parameters together 
...
\noindent such that Equation~\ref{eq:FOC_and_time_constraints} becomes
...
# Grab a subset of the data we want to present in this table.
usDataTable <- data[ , c("Y", "w_worker", "L", "L_A", "N", "B", "D", "K", "E")]
usDataTable$Y <- usDataTable$Y / 1e9 # Y is now in billion 2005$/year
usDataTable$L <- usDataTable$L / 1e6 # total employment mow in millions/year
usDataTable$L_A <- usDataTable$L_A / 1e6 # total STEM researchers now in millions/year
usDataTable$N <- usDataTable$N / 1e6 # total population now in millions
usDataTable$B <- usDataTable$B / 1e6 # births now in millions/year
usDataTable$D <- usDataTable$D / 1e6 # deaths now in millions/year
usDataTable$K <- usDataTable$K / 1e9 # capital stock now in billion 2005$
usDataTable$E <- usDataTable$E / 1e3 # energy now in TJ/year
colnames(usDataTable) <- c("$Y_\\mathrm{t}$", "$w_\\mathrm{t}$", "$L_\\mathrm{t}$", "$L_\\mathrm{A,t}$", 
                              "$N_\\mathrm{t}$", "$B_\\mathrm{t}$", "$D_\\mathrm{t}$", "$K_\\mathrm{t}$", 
                              "$E_\\mathrm{t}$")
rawDataXtable <- xtable(x=usDataTable, 
...
dataXtable <- xtable(x=data[, 10:23], 
                     caption="``Ground truth'' data for the U.S.", 
                     label="tab:Ground_truth_US_Data")
...
\begin{itemize}
\item{Mortality: $\omega_1$, $\omega_2$, and $\omega_3$ from 
      Equation~\ref{eq:mortality_rate}}
\item{Utility: $\eta$, $\gamma$, and $G$ from 
      Equation~\ref{eq:FOC_with_G}}
\item{Production:}
  \begin{itemize}
...
print("Mortality parameter estimation results.")
print("To be removed before submission.")
...
\noindent which can be used with the \texttt{R} \citep{R} function 
\texttt{nls} to estimate 
\begin{itemize}
  \item{$\omega_1~=~\Sexpr{pprint(omega_1, digits=1)}$~(deaths/year-capita)$^{-1}$}
  \item{$\omega_2~=~\Sexpr{pprint(omega_2, digits=3)}$}
  \item{$\omega_3~=~\Sexpr{pprint(omega_3, digits=2)}$~(deaths/year-capita)$^{-1}$}
\end{itemize}

\noindent $\omega_1$, $\omega_2$, and $\omega_3$ are all 
...
**** Martin: The curve fit here is a bit weird. The mortality function 
(Equation~\ref{eq:mortality_rate}) is from \citet{Jones:2001wn}. But the
best fit parameters yield a function that behaves strangely with 
regard to $z_\mathrm{t}$. Figure~\ref{fig:mortality_graph} shows that
at low $z_\mathrm{t}$ \emph{and} high $z_\mathrm{t}$, mortality decreases.
Somewhere in the middle, mortality is maximized. I would have expected 
a monotonically inverse relationship between $d_\mathrm{t}$
and $z_\mathrm{t}$, i.e., $d_\mathrm{t}$ decreases as $z_\mathrm{t}$ 
increases. 
But maybe my assumptions are wrong here.
I note that our parameter values are significantly different
from Jones' values. In particualar, our value of $\omega_2$ is 
more negative than his. --Matt ****

**** Martin says:

Assuming that all equations are correctly reproduced, 
the anomaly must lie in the short time period of the data when compared what Jones did. 

1. could it possibly have been the case that 
subsistence levels increased after 1980 in the US before declining again? 
(See Jones, p5: ``The mortality rate dt is assumed to be a function 
of the average level of per capita consumption relative to subsistence''

2. another thought. It is at subsistence level that people die according to model.  
Actual mortality numbers are in fact driven by much more than subsistence only, 
especially in a country like the US, post 1980 and highly developed, 
the signal wrt subsistence can be expected to be weak. 

3. is it conceivable that mortality could actually increase 
with increased consumption over subsistence level as seen from z=2.0 to z=2.5?  
I think it can--new kinds of diseases for example. 
But it still does not explain why only up to certain 
consumption levels--we need to unpack the data to better understand this.
****

<<mortality_graph, fig.pos="H", fig.width=singleGraphWidth, fig.height=singleGraphHeight, fig.cap="Mortality model results.">>=
d_pred <- predict(dModel)
d_act <- data[ , "d"]; names(d_act) <- "d_act"
dGraphData <- as.data.frame(cbind(d_act, d_pred, data$z))
names(dGraphData) <- c("d_act", "d_pred", "z")
...
print("Utility parameter estimation results.")
print("To be removed before submission.")
print(summary(uModel))
@

Utility model parameters $\eta$, $\gamma$, and $G$ 
are estimated a least squares technique. Applying 
Equation~\ref{eq:FOC_with_G} at 
$\mathrm{t} = \mathrm{t_0} = 1980$ and solving 
for $G$ yields

\begin{equation} \label{eq:G_t0}
  G = w_0 \; \frac{\tilde{b}_0^\eta}{\tilde{c}_0^\gamma}
\end{equation}

Substituting Equation~\ref{eq:G_t0} into Equation~\ref{eq:FOC_with_G}
ensures that year $\mathrm{t} = \mathrm{t}_0$ information 
will be matched perfectly. Solving for $\tilde{b}$ gives
...
\texttt{nls} to estimate 
\begin{itemize}
  \item{$\eta = \Sexpr{pprint(eta, digits=3)}$}
  \item{$\gamma = \Sexpr{pprint(gamma, digits=3)}$}
  \item{$G = \Sexpr{pprint(G, digits=3)}$}
\end{itemize}

\noindent All fitted parameters are significant at the 0.1\% level.

**** Martin: Again, the results are a bit surprising. 
\citet{Jones:2001wn} indicates that $\eta$, $\gamma$ $\in$ (0,1).
Our best fit to historical data from the U.S. for the 
years 1980--2011 gives negative values for both coefficients. 
Do you see anything that we are doing wrong in our analysis? 
Is there a way to ``fix'' our analysis? 
Should we identify an alternative way to describe 
the choices between childrearing and working? --Matt ****

****

Martin says: Jones (p3) simply assume those ranges: 
"We assume 0.0 $\le \mu \le$ 1.0, 0.0 $\le \gamma \le $ 1.0, and 0.0 $\le \eta \le$ 1.0. 
These parameter restrictions ensure that the elasticity of 
substitution between consumption ($\tilde{c}$) and children 
($\tilde{b}$) is always greater than one. 
This simple assumption will play an important role 
in generating the demographic transition.

Now, the period of data and the developmental state of the US 
needs no fitting of a demographic transition. 
That has already happened in the US. 
I see two ways fwd: 
(i) Place those restrictions on the model and see how it behaves. 
(ii) Loosen up those restrictions as is done so far, but explain what that means.
****

**** Martin: This is as far as you need to read. --Matt ****


<<utility_graph, fig.pos="H", fig.width=singleGraphWidth, fig.height=singleGraphHeight, fig.cap="Mortality model results.">>=
xyplot(predict(uModel)~data$b_tilde, #calculatedHistoricalData["b_tilde"], 
       xlab=list(label="Actual $\\tilde{b}$", cex=textScaling), 
       ylab=list(label="Predicted $\\tilde{b}$", cex=textScaling),
       scales=list(cex=scaleTextSize * textScaling, # controls text size on scales.
                   tck=scaleTickSize # controls tick mark length. < 0 for inside the graph.
                   ),
       lty=lineTypes, lwd=lineWidths, col=colors, # controls line parameters
