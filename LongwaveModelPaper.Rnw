...
``historical'' time series for other important parameters 
in the model. To obtain $b_\mathrm{t}$ in units of 
...
c <- calculatedHistoricalData["w_worker"] * tau; names(c) <- "c" # consumption in 2005$/year-capita. Same as wages in 2005$/year-capita.
...
Following \citet[p.~16]{Jones:2001wn}, we take $\bar{b} = 0$ 
for the asymptotic birth rate as consumption goes to infinity.
...
The value of $\bar{c}$ was set to be 5655 2005\$/year-capita, 
based on the poverty line value in the year 1980 and 
assuming an average family size of 3.

<<US_data_table, results='asis'>>=
dataXtable <- xtable(x=calculatedHistoricalData[, 10:20], caption="Historical data for the U.S.", label=paste("tab:Historical_US_Data"))
