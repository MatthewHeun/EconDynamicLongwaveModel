...
We used $\tau_\mathrm{1980}$ and $b_\mathrm{1980}$ and Equation~\ref{eq:birth_constraint} to calculate a value for $\nu$ (\Sexpr{nu_0}~births/year-capita) that is held constant through time. We used $L_\mathrm{A,1980}$ and $L_{1980}$ to calculate an invariant value for $\pi$ (0.006518). $\pi$ was used along with the historical wage and labor data to calculate an invariant value for $\zeta$ (0.6794) **** Heun Write R code to calculate these numbers. ****
...
dataXtable <- xtable(x=calculatedHistoricalData[, 10:18], caption="Historical data for the U.S.", label=paste("tab:Historical_US_Data"))
