...
$U_{\mathrm{c,0}}$ and $U_{\mathrm{b,0}}$ are the utility derived from 
  ...
  at $\mathrm{t} = t_\mathrm{0}$ (in units of utils/year). 
...
we calculated ``historical'' time series for other important parameters 
...
$\nu$ (\Sexpr{nu_1980}~births/year-capita) that is 
...
dataXtable <- xtable(x=calculatedHistoricalData[, 10:20], 
                     caption="Historical data for the U.S.", label=paste("tab:Historical_US_Data"))
...
we estimate values for the fitting parameters, including 
\begin{itemize}
\item{$\beta$ and $\sigma$ from Equation~\ref{eq:Jones_production_function};}
\item{$\bar c$ and $\bar b$ from Equations \ref{eq:c_tilde} and 
  \ref{eq:b_tilde}, respectively;}
\item{$\nu$ from Equation \ref{eq:birth_constraint};}
\item{$\eta$, $\gamma$, and $G$ from Equation~\ref{eq:FOC_with_G};}
\item{$\zeta$ from Equation~\ref{eq:knowledge_compensation};}
\item{$\pi$ from Equation \ref{eq:pi};}
\item{$\lambda$ and $\phi$ from Equation \ref{eq:da_dt};}
\item{$\delta$ and $A_\mathrm{0}$ from Equation \ref{eq:delta};}
\item{$m$ from Equation \ref{eq:dn_dt}; and}
\item{$\omega_1$, $\omega_2$, $\omega_3$, and $\bar d$ from 
  Equation~\ref{eq:mortality_rate}.}
\end{itemize}
...
To estimate the parameters outlined in Section 
\ref{sec:basic_model_parameter_estimation}, a least-squares 
minimization technique was used on different portions 
of the model. For the purposes of our estimation techniques, 
we can decompose the model into three parts: 
mortality, 
utility function, and 
production. 
We discuss each portion below.
...
Knowing historical values for $c_\mathrm{t}$ and $\bar{c}$ 
allowed us to calculate historical values of $z_\mathrm{t}$. 
These historical $z_\mathrm{t}$ values were used to calculate 
a predicted $d_\mathrm{t}$ value that was then compared to 
the historical $d_\mathrm{t}$ values for the minimzation. 
To determine values of the three mortality function fitting 
parameters ($\omega_\mathrm{1}$, $\omega_\mathrm{2}$, 
and $\omega_\mathrm{3}$), we used least-squares minimization 
when comparing the predicted $d_\mathrm{t}$ to the historical 
values. Because you have two equations and three unknowns, one 
of the $\omega$ values can be allowed to float. For the purposes 
of our parameter estimation, we let $\omega_\mathrm{2}$ float 
and performed the least-squares minimzation on 
$\omega_\mathrm{1}$ and $\omega_\mathrm{3}$.
