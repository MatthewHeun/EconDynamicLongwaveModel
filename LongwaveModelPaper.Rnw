...
l_Y <- (data["L_Y"]) / L_Y_0; names(l_Y) <- "l_Y" # Indexed workers in the manufacturing sector.
data <- cbind(data, y, n, l_Y)
@
...
tau <- data["L"] / data["N"]; names(tau) <- "tau" # Fraction of workers in each year
w_worker <- data["w_capita"] / tau; names(w_worker) <- "w_worker" # wage per worker
c <- w_worker * tau; names(c) <- "c" # consumption per capita
data <- cbind(data, tau, w_worker, c)
@
...
<<US_data_table, results='asis', label="tab:US_Data">>=
dataXtable <- xtable(x=data[, 10:18], caption="Data for the U.S.", label=paste("tab:US_Data"))
